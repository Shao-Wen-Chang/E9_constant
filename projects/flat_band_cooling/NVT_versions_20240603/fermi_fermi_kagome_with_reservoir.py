...
T = 0.1             # (fundamental) temperature (k_B * T)
...
E_range_s = (0, bandwidth + 1)    # energies considered in calculation
    # Defining DoS
DoS_sys = lambda x: util.kagome_DoS(x, hbw = bandwidth / 2)
...
nu_sf2 = 5/12               # filling factor (1 is fully filled)
Np_sf2 = int(V * r_s * nu_sf2)        # number of particles in the system

exp_ffkwr = E9M.NVT_exp(T, [
...
def calc_isentropic():
    ...
    # Additional inputs
    isen_S_list = np.arange(2000, 15000, 1000, dtype = float)
    ...
    for i, S_now in enumerate(isen_S_list):
        ...
        isen_T_list[i], isen_rrst_list[i] = eqfind.NVT_from_NVS_solver(S_now, exp_ffkwr)
    ...
    # Sample run
    fig_sam = plt.figure(2, figsize = (10, 16))
    sampled_run = 6
...
def calc_canonical():
    ...
    equilibrium conditions."""
    logging.warning("This mode is not working yet!")
    # Additional inputs
    ...
    for i, S_now in enumerate(isen_S_list):
        ...
        isen_TN_list[i], isen_rrst_list[i] = eqfind.isentropic_fix_Ntot_solver(N_total, S_now, exp_ffkwr)
