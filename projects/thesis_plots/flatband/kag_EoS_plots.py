# adapted from fermi_kagome_scan_reservoir.py
import sys
...
sp_name = "fermi1"
T = 0.2             # Temperature
T_scan = np.linspace(0.025, 1, 40)# + 0.01145 # fine tuned by hand to match \sigma for the mu = 1.6 plot
mu = 4.5              # chemical potential (used 1.5 and 1.6 for plots)
mu_scan = np.arange(13, 18) / 10.

# Utility variables
calculation_mode = "thesis_plot" # "T_scan" # "T_mu_scan" # 
...
def calc_T_scan():
    ...
    # Sample run
    # sampled_runs = [3]  # mu = 1.6
    sampled_runs = [0, 7]  # mu = 1.5
    sampled_runs = [5, 15]  # mu = 4.5
    for sr in sampled_runs:
        # vvvvvvvvvvvvvvv This is the bigger plot with table below vvvvvvvvvvvvvvv
        fig_sam, (ax_DoS, ax_tab) = plt.subplots(2, 1, figsize = (5,8))
        exp_list[sr].plot_DoSs(ax_DoS, offset_traces = 0.5)
        if xlim_DoS is not None: ax_DoS.set_xlim(xlim_DoS)

        ax_tab.set_axis_off()
        exp_list[sr].tabulate_params(ax_tab)
        fig_sam.suptitle("Showing T = {:.2f}".format(T_scan[sr]))
        # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        fig, ax = plt.subplots(1, 1, figsize = (4, 3.5))
        exp_list[sr].plot_DoSs(ax, offset_traces = 0.5)
        N_sam, S_sam = N_tot_arr[sr], S_tot_arr[sr]
        sig = S_sam / N_sam
        ax.set_title(rf"$\sigma = {sig:.4f}$")
        ax.set_yticks([])   # for mu = 1.5
        ax.set_ylabel("")   # for mu = 1.5

        # thesis_fig_path = Path(r"C:\Users\ken92\Documents\Studies\E5\Theses\figures\flatband")
        # file_full_path = Path(thesis_fig_path, f"kag_EoS_Voff{offset}_mu{mu}_sig{sig:.4f}.pdf")
        # fig.savefig(file_full_path, facecolor = "none", dpi = 300, bbox_inches='tight')

def calc_thesis():
    """Find the thermodynamical values at some fixed mu and offset while scanning T."""
    # Initialization
    # I don't need to go through all the badly documented initialization
    mus = [1.6, 1.5, 1.5]
    Ts = [0.11145, 0.025, 0.2]
    # Ts = [0.11145, 0.05, 0.1] # To see that 0.28 is roughly the threshold value
    exp_list = [None for _ in mus]

    # Calculation (done when an muVT_exp is instantiated)
    for i, (mu, T) in enumerate(zip(mus, Ts)):
        logging.debug("working on loop #{}, T = {}...".format(i, T))
        exp_list[i] = E9M.muVT_exp(T, sr_list, {sp_name: mu})
    S_tot_arr = np.array([ex.S for ex in exp_list])
    N_tot_arr = np.array([ex.N_dict[sp_name] for ex in exp_list])
    ...
    for i, (mu, ax) in enumerate(zip(mus, axes)):
        exp_list[i].plot_DoSs(ax, offset_traces = 0., l_dirac = 1.)
        N_sam, S_sam = N_tot_arr[i], S_tot_arr[i]
        sig = S_sam / N_sam
        ax.set_title(rf"$\mu = {mu}, \sigma = {sig:.4f}$")
    ...
    thesis_fig_path = Path(r"C:\Users\ken92\Documents\Studies\E5\Theses\figures\flatband")
    file_full_path = Path(thesis_fig_path, "kag_DoS_examples.pdf")
    # fig.savefig(file_full_path, facecolor = "none", dpi = 300, bbox_inches='tight')
