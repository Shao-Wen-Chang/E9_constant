...
#%% Plot related stuff
def get_Vin(x_, y_, V, r1 = 1, r3 = 1, dx = 0, dy = 0, lamb = 1):
    """Get in-plane polarization potential (lamb stands for lambda)"""
    x, y = x_ - dx, y_ - dy
    xx, yy = np.meshgrid(x, y)
    return (2/9) * V * (3 - r1 *      np.cos((E9c.g1[0] * xx + E9c.g1[1] * yy) / lamb)
                          - r1 * r3 * np.cos((E9c.g2[0] * xx + E9c.g2[1] * yy) / lamb)
                          -      r3 * np.cos((E9c.g3[0] * xx + E9c.g3[1] * yy) / lamb))

def get_Vout(x_, y_, V, r1 = 1, r3 = 1, dx = 0, dy = 0, lamb = 1):
    """Get out of plane polarization potential"""
    x, y = x_ - dx, y_ - dy
    xx, yy = np.meshgrid(x, y)
    return (1/9) * V * (3 + 2 * r1 *      np.cos((E9c.g1[0] * xx + E9c.g1[1] * yy) / lamb)
                          + 2 * r1 * r3 * np.cos((E9c.g2[0] * xx + E9c.g2[1] * yy) / lamb)
                          + 2 *      r3 * np.cos((E9c.g3[0] * xx + E9c.g3[1] * yy) / lamb))

def get_Vsuper(x, y, V532, Pol532, V1064, Pol1064, dx, dy, r1_532 = 1, r3_532 = 1, r1_1064 = 1, r3_1064 = 1):
    """Get combined potential (Pol stands for polarization)"""
    return Pol532(x, y, V532, r1_532, r3_532, 0, 0, 1) - Pol1064(x, y, V1064, r1_1064, r3_1064, dx, dy, 2)
...
def PlotBZSubplot(ax_BZ: plt.axes = None,
    ...
    if label_sym_pts:
        ...
        ax_BZ.text(0, -0.2, r"$\Gamma$", fontsize = fontsize, va = "top", ha = "center")
        # ax_BZ.text(0, -0.1, r"$\mathbf{\Gamma}$", fontsize = fontsize, va = "top", ha = "center")

        ax_BZ.scatter(np.sqrt(3)/2, 0, **sym_pt_dict)
        ax_BZ.text(np.sqrt(3)/2 + 0.1, 0, "M", fontsize = fontsize, va = "center")
        ...
        ax_BZ.text(0, 1 - 0.2, "K'", fontsize = fontsize, va = "top", ha = "center")
